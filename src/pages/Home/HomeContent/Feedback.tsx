import { useRef } from "react";
import { Navigation } from "swiper/modules";
import { Swiper, SwiperSlide } from "swiper/react";
import logo from "../../../assets/images/feedback_logo.png";
import "swiper/css";
import FeedbackCard from "../../../components/FeedbackCard";
import SliderArrowBtn from "../../../components/SliderArrowBtn";
import { motion } from "framer-motion";

type SwiperType = {
  swiper: {
    activeIndex: number;
    slideTo: (index: number) => void;
  };
};

const feedbacks = [
  {
    id: 1,
    name: "Tristan Parker",
    logo: logo,
    description:
      "Working with Samiul was a game-changer. He transformed our website's user experience, making it intuitive and visually stunning. Our customers love it!",
  },
  {
    id: 2,
    name: "Ava Mitchell",
    logo: logo,
    description:
      "Working with Samiul was a game-changer. He transformed our website's user experience, making it intuitive and visually stunning. Our customers love it!",
  },
  {
    id: 3,
    name: "Ethan Bennett",
    logo: logo,
    description:
      "Working with Samiul was a game-changer. He transformed our website's user experience, making it intuitive and visually stunning. Our customers love it!",
  },
  {
    id: 4,
    name: "Lily Thompson",
    logo: logo,
    description:
      "Working with Samiul was a game-changer. He transformed our website's user experience, making it intuitive and visually stunning. Our customers love it!",
  },
  {
    id: 5,
    name: "Sophia Nguyen",
    logo: logo,
    description:
      "Working with Samiul was a game-changer. He transformed our website's user experience, making it intuitive and visually stunning. Our customers love it!",
  },
  {
    id: 6,
    name: "Ethan Bennett",
    logo: logo,
    description:
      "Working with Samiul was a game-changer. He transformed our website's user experience, making it intuitive and visually stunning. Our customers love it!",
  },
];

const Feedback = () => {
  const swiperRef = useRef<SwiperType | null>(null);

  const handleScroll = (event: React.WheelEvent<HTMLDivElement>) => {
    event.preventDefault();
    if (swiperRef.current) {
      const swiper = swiperRef.current.swiper;
      swiper.slideTo(swiper.activeIndex + Math.sign(event.deltaY));
    }
  };

  return (
    <div
      className="md:pt-[60px] pt-[40px] bg-[#2B2E4A1A] dark:bg-dark_secondary"
      id="testimonial"
    >
      <div className="text-center max-w-[802px] mx-auto">
        <motion.h2
          initial={{ scale: 0.9, opacity: 0 }}
          whileInView={{ scale: 1, opacity: 1 }}
          transition={{ duration: 0.6 }}
          viewport={{ once: true }}
          className="md:text-2xl text-[14px] font-medium text-text_primary dark:text-text_secondary"
        >
          Client Voices
        </motion.h2>
        <motion.h1
          initial={{ scale: 0.9, opacity: 0 }}
          whileInView={{ scale: 1, opacity: 1 }}
          transition={{ duration: 0.8 }}
          viewport={{ once: true }}
          className="md:text-[56px] text-2xl font-semibold md:leading-[67.2px] leading-[28.8px] text-secondary dark:text-white"
        >
          Real Feedback and Success Stories from My Projects
        </motion.h1>
      </div>
      <section className="md:py-[60px] py-10" onWheel={handleScroll}>
        <div className="container-overflow">
          <Swiper
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            // @ts-ignore
            ref={swiperRef}
            modules={[Navigation]}
            spaceBetween={20}
            slidesPerView={"auto"}
            navigation={{
              prevEl: ".feedback-prev",
              nextEl: ".feedback-next",
            }}
            breakpoints={{
              640: {
                spaceBetween: 35,
              },
            }}
          >
            {feedbacks.map((feedback) => (
              <SwiperSlide
                className="w-[305px] h-[300px] pt-10 lg:w-80"
                key={feedback.id}
              >
                <FeedbackCard feedback={feedback} />
              </SwiperSlide>
            ))}
          </Swiper>
        </div>
        <motion.div
          initial={{ scale: 0.9, opacity: 0 }}
          whileInView={{ scale: 1, opacity: 1 }}
          transition={{ duration: 0.9 }}
          viewport={{ once: true }}
          className="flex items-center justify-center gap-2 sm:gap-5 pt-5 "
        >
          <SliderArrowBtn direction="left" className="feedback-prev" />
          <SliderArrowBtn direction="right" className="feedback-next" />
        </motion.div>
      </section>
    </div>
  );
};

export default Feedback;
